...
template <typename T> void __header_print(uint16_t fields, std::vector<const char*> &values,std::vector<int> &widths, STYLE style = S_NONE){
  PDEBUG(1, "__header_print(template)");
  ...
  if(std::is_base_of_v<T,Profile>){
    for(int i = 0; i < values.size(); ++i){
      ...
      int &w = widths[i];
      fields & pfield_arr[i] && entry << fmt::format("{:^" + std::to_string(w) + "}",v);
    }
  }else if(std::is_base_of_v<T,Entity>){
    for(int i = 0; i < values.size(); ++i){
      const char *v = values[i];
      int &w = widths[i];
      fields & efield_arr[i] && entry << fmt::format("{:^" + std::to_string(w) + "}",v);
    }
  }
...
static inline void __table_print(PROFILE_FIELDS &pfields, std::map<std::string_view,Profile>* profiles, std::vector<int> &widths, STYLE style = S_NONE){
  PDEBUG(1,"__table_print(Profile)");
...
// @param fields desired PROFILE_FIELDS or ENTITY_FIELDS to show, obtained via -ef -pf on `./gpkih list` action
template <typename T> void __table_print(ENTITY_FIELDS &fields, const std::map<std::string_view,T>* data, std::vector<int> &widths, STYLE style = S_NONE){
  PDEBUG(1,"__table_print(Entity)");
...
int actions::list(std::string_view profile_name, PROFILE_FIELDS pfields, ENTITY_FIELDS efields) {
  PDEBUG(1, "actions::list()");
  // Check if existing_profiles is empty
  auto existing_profiles = db::profiles::get();
  date_format = Config::get("formatting","date_format");
  date_width = static_cast<int>(std::max((size_t)19,fmt::format(date_format,std::chrono::system_clock::now()).size() + 2));

  PDEBUG(1,"date format: {} date width: {}", date_format, date_width);
  if (profile_name.empty()) {
    ...
    // Print headers
    __header_print<Profile>(static_cast<uint16_t>(pfields), headers, widths, fg(T_GREEN) | EMPHASIS::bold);
    // Print profiles
    __table_print(pfields, existing_profiles, widths);
  }else{
    ...
    std::vector<int> widths{8,13,date_width,8,9,9,7,10,14,7,10,10,10};
    ...
    // Print headers
    __header_print<Entity>(static_cast<uint16_t>(efields), headers, widths, fg(T_GREEN) | EMPHASIS::bold);
    // Print entities
    __table_print(efields, entity_list, widths);    
  }
